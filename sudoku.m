...
function zkontroluj (fig, umisteni, cudlik)
    ...
    if ~lzeVlozit(fig, umisteni, prvek) % true only if gets 0 (= 0 failed constrainces)
       set(cudlik, 'BackgroundColor', 'w'); 
       matice(umisteni(1), umisteni(2)) = prvek; %save into matrix
    else
        set(cudlik, 'BackgroundColor', 'r'); % wrong number => red background colour
    end
    DATA{1,2} = matice;
    DATA{1,1} = handle;
    set(fig,'userdata',DATA)
...
function [povoleni] = lzeVlozit (fig, pozice, prvek)
    if prvek > 0 && prvek < 10   % check if the number is from range <1,9>
        DATA = get(fig, 'userdata');
        matice = DATA{1,2};
        ohodnoceni = ismember(matice, prvek);
        radky = sum(ohodnoceni, 2);
        radek = radky(pozice(1)); % chceck repetition of the number in row
        sloupce = sum(ohodnoceni, 1);
        sloupec = sloupce(pozice(2)); %check repetition of the number in column
    
        if ismember([1,2,3], pozice(1))  %% find right square
            if ismember([1,2,3], pozice(2))
                ctverec = sum(sum(ohodnoceni(1:3,1:3)),2);
            elseif ismember([4,5,6], pozice(2))
                ctverec = sum(sum(ohodnoceni(4:6,1:3)),2); % chceck repetition of the number in the square
            else
        ...
        povoleni = radek + sloupec + ctverec; % sum of violations
    else
        povoleni = 1; % in case of number out of range
    end
end
...
% choose right handler according the position in the grid
    DATA = get(fig, 'userdata');
    handle = DATA{1,1};
   switch pozice(1)
       case 1
